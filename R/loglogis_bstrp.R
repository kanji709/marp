...
#'
#' @return returns list of estimates after fitting Log-Logistic renewal model on (double) bootstrap samples
#' \describe{
...
loglogis_bstrp <- function(n, t, B, BB, m, par_hat, mu_hat, pr_hat, haz_hat, y) {
  ## bootstraps
  bstrp <- replicate(B, VGAM::rfisk(n, par_hat[9], par_hat[3]))
  ## fit a Log-Logistic renewal model
  star <- apply(bstrp, 2, function(x) loglogis_rp(x, t, m, y))
  ## parameters
  ...
  ## double-bootstraps & fit a Log-Logistic renewal model
  double <- sapply(1:B, function(i) replicate(BB, VGAM::rfisk(n, par2_star[i], par1_star[i])), simplify = "array")
  star_double <- apply(double, c(2, 3), function(x) loglogis_rp(x, t, m, y))
  ...
  mu_Tstar <- (mu_star - mu_hat[3]) / sqrt(mu_var_double)
  pr_Tstar <- (pr_star - pr_hat[3]) / sqrt(pr_var_double)
  haz_Tstar <- (haz_star - haz_hat[, 3]) / sqrt(haz_var_double)
