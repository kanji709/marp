...
#'
#' @return returns list of estimates after fitting Poisson renewal model on (double) bootstrap samples
#' \describe{
...
poisson_bstrp <- function(n,t,B,BB,par_hat,mu_hat,pr_hat,haz_hat,y) {
  ## bootstraps
  bstrp <- replicate(B, stats::rexp(n, par_hat[1]))
  ## fit a Poisson renewal model
  star <- apply(bstrp, 2, function(x) poisson_rp(x, t, y))
  ## parameters
  par_star <- sapply(star, '[[', 1)
  ...
  ## double-bootstraps
  double <- sapply(1:B, function(i) replicate(BB, stats::rexp(n, par_star[i])), simplify = "array")
  ## fit a Poisson renewal model
  star_double <- apply(double, c(2, 3), function(x) poisson_rp(x, t, y))
  ...
  mu_Tstar <- (mu_star - mu_hat[1]) / sqrt(mu_var_double)
  pr_Tstar <- (pr_star - pr_hat[1]) / sqrt(pr_var_double)
  haz_Tstar <- (haz_star - haz_hat[, 1]) / sqrt(haz_var_double)
  return(list(
    'mu_star' = mu_star,
    'pr_star' = pr_star,
    'haz_star' = haz_star,
